...
window.__I18N_EN__ = {
  ...
  "issue": {
    ...
    "checks-unverified": {
      "msg": "Some checks could not be completed ({0}) — these are shown as unknown, not as failures.",
      "what": "One or more DNS lookups for this domain did not return a usable answer, usually a SERVFAIL or a timeout from the resolver. That can be a genuinely broken nameserver, but it is just as often a transient hiccup that clears within minutes. The rest of the audit completed normally and is accurate. The affected checks are reported as unknown and excluded from the score rather than counted as zero, which is why the grade may be shown as a range — scoring an unanswered query as a missing record would lower your grade for our failure, not yours.",
      "fix": "Re-run the audit. If the same checks keep failing, query the affected names directly to see whether your nameservers answer them. A server that returns SERVFAIL instead of NXDOMAIN for names that do not exist will trip this repeatedly.",
