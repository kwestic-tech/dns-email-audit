...
Six phases. **Gates 0–5 are met; Gate 6 — the release — is not**, and Task 6.9
is what cuts it. **Thirteen owning directories** under `src/`, each with its own
`API.md` and co-located tests — `audit/`, eight protocol owners, `core/dns/`,
`core/shared/`, `providers/` and `ui/`; one delivery boundary
