...
| Field | Value |
| --- | --- |
| Spec version | 1.9 (Implemented, amended) |
| Target release | 0.6.0 |
| Status | **Released as `v0.6.0` on 2026-08-30.** All six gates are met. Originally **Final.** Approved for implementation after three Codex review rounds. Amended to `1.1` during Phase 2 to correct one incomplete enumeration in §11, to `1.2` to correct an overstated evidence claim about how that enumeration is guarded, to `1.3` to add the last ambient primitive the conversion sweep found, and to `1.4` — during Task 2.7 — to state the oracle's provenance once the supported facade replaces the legacy engine global and reclassify the PSL fixture-identity fingerprint as binding-level, to `1.5` at the Gate 2 audit to correct two claims `1.4` overstated, to `1.6` at Task 3.6 to replace §3's three-name exception-edge row with two separate inventories, to `1.7` and `1.8` at implementation completion and release, and to `1.9` during 0.9.0's commit 4 to add `nowIso()` to §11's primitive set. See [Revision history](#revision-history). Linux dependency installation was verified on a native Linux runner at Gate 1. |
...
`FileReader`, `Intl`, `console`, `now()`, `formatDateTime(date, locale)` and
`nowIso()`. `now()` and `formatDateTime()` preserve the current
`new Date().toLocaleString(i18n.lang)` behavior in production while making
export parity deterministic. `nowIso()` is here as of spec `1.9`, added for
0.9.0's report timestamp: that value is machine-read and must be identical
between two exports of one run, which a localized format is not. All three
keep the clock a capability the runtime holds, so the equivalence runner's
pinned `Date` controls them and no module above reaches for an ambient one. Language
...
| Version | Date | Change |
| --- | --- | --- |
| 1.9 | 2026-09-03 | **Added `nowIso()` to §11's primitive set, during 0.9.0's commit 4.** The report schema's `generatedAt` is a machine-read UTC instant that must be byte-identical between two exports of one run, so neither `now()` (milliseconds) nor `formatDateTime()` (localized text) can supply it. The alternative was an ambient `Date` read in `src/ui/`, which is the boundary breach 0.9.0's own review had just caught once in `report-data.js` and which the lexical ambient catalog does not name -- so the spec is amended rather than the implementation bent, the same disposition as `navigator` at `1.1` and `open` at `1.3`. Added to §11, `PLATFORM_PRIMITIVES` and `SPEC_11`. No architecture, phase ordering or acceptance threshold reopened, and no 0.6.0 surface moves: the equivalence run over the 32-case corpus reports zero differences across all five surfaces with it in place. |
