 ...
 * `core/shared/` only.
 *
 * **Ruled at Task 4.0, finding 4, and completed at Task 5.2.** Task 4.9
 * injected `isNullMx` — the PREDICATE — because the ruling's end state needed
 * an `src/audit/` that did not exist yet. It exists now, so this module
 * receives the derived FACT instead: `detectEmailProvider()` takes `nullMx` as
 * a fourth argument and reads it. Audit derives the boolean once, with
 * `core/mx/`'s own predicate, and reads it twice — here and at its
 * deep-check gate.
 *
 * With the predicate gone there is no capability left to close over, so the
 * factory is gone too: three pure functions, which is what §12's table said
 * this directory was. A factory that injects nothing is a false statement
 * about what a module needs.
 *
...
export function detectEmailProvider(mx, domain, addressRecords, nullMx) {
  // The DERIVED fact, supplied by audit. The null-MX test comes FIRST, so a
  // `0 .` domain that also publishes addresses is not read as implicit MX.
  if (nullMx) return '@null-mx';
...
export function detectHosting(aRecs, wwwCname, domain) {
  const a = aRecs.join(' ');
