...
export const LOCALE_EN = {
  ...
  "issue": {
    ...
    "mx-vanity-divergent": {
      "msg": "MX host is missing addresses its provider publishes: {0} does not publish every globally reachable address {1} does — mail for this domain never reaches {2}.",
      "what": "This MX host is named inside your own domain, but the address record behind it is a copy of a hosted provider's. That copy is a fork: it does not follow the provider when they renumber, and it does not have to contain every address they publish. Right now it does not.\n\nThe practical effect is fewer delivery paths than you think you have. Your provider publishes several reachable addresses; your domain offers senders only the ones your copy lists. If an address you did copy stops answering, mail queues at the sender even though another published address was still reachable. What DNS shows is addresses, not machines — those addresses may or may not be separate servers — so what you are missing is address paths, not proven independent capacity. The failure is also silent from your side: inbound mail failures produce bounces at the sender, not alerts for you.\n\nThe provider name here is taken from the reverse DNS of your own MX address and then confirmed by resolving that name back to the same address. That round trip is evidence of a relationship between your address and that name; it does not establish who owns or operates the address, and this tool does not claim to.\n\nOnly globally reachable addresses are compared. An address in private, shared, documentation or IPv4-mapped space cannot accept mail from the Internet, so it is never counted as redundancy you are missing and never named above.",
      "fix": "Start with your provider's documentation, not with the name above. The provider name shown here comes from reverse DNS and was confirmed by resolving it back to the same address; that proves a mapping exists, not that the name is a documented, stable MX target your account is entitled to use. It is frequently a generic per-address name, and pointing your MX at one can replace a working configuration with a name the provider never intended to serve you.\n\nSo: if your provider designates a hostname as the MX target for your account, use that name — theirs, from their documentation, not this one — and let them manage the addresses behind it. If they do not, keep your own name and reconcile the addresses with them: ask which globally reachable addresses your MX should publish, and add those. The addresses named above are what your host currently lacks and are the right thing to ask about. Add only addresses reachable from the Internet: publishing a private or otherwise reserved one creates the unroutable-address fault this tool reports separately. Lower the record's TTL to match the provider's if you keep a copy — it remains a copy, and it will drift again.",
      "fixCode": "; Before — a name in your zone holding one of the provider's addresses.\n; 203.0.113.10 is an example value only: substitute your own public address.\nexample.com.        MX     10 mail.example.com.\nmail.example.com.   A      203.0.113.10\n\n; After — ONLY if your provider documents this name as your MX target.\n; mx.provider.example is a placeholder for that documented name, not the\n; name this audit inferred from reverse DNS.\nexample.com.        MX     10 mx.provider.example."
    },
    "mx-no-reverse-dns": {
      "msg": "No reverse DNS on the checked addresses of {0} — advisory only for a receiving host.",
      "what": "Reverse DNS maps an address back to a name. None of the addresses checked for this host published one.\n\nUp to four unique addresses per host are checked. The A answers are considered first, then the AAAA answers, each in the order the resolver returned them — so a host publishing four or more IPv4 addresses has its IPv6 addresses left unchecked. A host with more addresses than that can have reverse DNS on one that was not checked, so this says what was observed rather than that the host has none anywhere.\n\nOn the receiving side that is a hygiene note and not a delivery risk, which is why this is advisory: RFC 5321 §4.1.4 says explicitly that a failed reverse lookup should not on its own be grounds for refusing a message, and receivers do not enforce it against the hosts they deliver to.\n\nWhere reverse DNS genuinely matters is the opposite direction — the address your mail is sent *from*. Large receivers require a sending IP to have valid reverse DNS, and mail from one that does not is commonly rejected or heavily penalised. This tool audits the records a domain publishes, not its egress, so it cannot tell you whether your sending addresses are configured; it can only tell you about the hosts named here.",
      "fix": "Nothing is required for inbound mail. If this host also sends, ask whoever holds the address block to publish a reverse record for it, and make sure that name resolves back to the same address — receivers check the round trip, not just the presence of a name."
    },
