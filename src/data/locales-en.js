...
export const LOCALE_EN = {
  ...
  "issue": {
    ...
    "mx-vanity-divergent": {
      "msg": "MX host is missing addresses its provider publishes: {0} resolves to fewer addresses than {1} — {2} is unreachable through this domain.",
      "what": "This MX host is named inside your own domain, but the address record behind it is a copy of a hosted provider's. That copy is a fork: it does not follow the provider when they renumber, and it does not have to contain every address they publish. Right now it does not.\n\nThe practical effect is lost redundancy you believe you have. Your provider is advertising more than one mail server; your domain reaches only the ones your copy lists. If the address you did copy goes down, mail queues at the sender even though a working server was available the whole time. The failure is also silent from your side — inbound mail failures produce bounces at the sender, not alerts for you.\n\nThe provider name here is taken from the reverse DNS of your own MX address and then confirmed by resolving it back, so it is the operator of the address you published rather than a guess.",
      "fix": "Point the MX record at the provider's own hostname instead of a name in your zone. That hands address management to the party who knows when the addresses change, and picks up every server they publish. If a branded name is required, copy all of the provider's current addresses and lower the record's TTL to match theirs — but it remains a copy, and it will drift again.",
