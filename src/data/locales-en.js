...
export const LOCALE_EN = {
  ...
  "issue": {
    ...
    "mx-invalid-preference": {
      "msg": "MX preference value is outside the range the record format allows: {0} — the maximum is 65535.",
      "what": "The preference in an MX record is a 16-bit unsigned integer, so RFC 1035 §3.3.9 gives it a range of 0 to 65535. A larger number cannot be represented in the record as it goes on the wire. What happens to it depends on whoever wrote the zone: some DNS providers reject the value, some truncate it, and some accept the text and publish something other than what was typed. In the last case the preference ordering you intended is not the ordering senders see, which matters as soon as one host is meant as a fallback.\n\nThis does not by itself stop mail being delivered, and it is reported as hygiene rather than as a fault for that reason.",
      "fix": "Renumber the preferences into the valid range. Only their relative order matters, so conventional steps of 10 leave room to insert a host later without renumbering the rest."
    },
