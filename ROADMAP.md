...
| 11 | MX host address validity and provider divergence | **0.9.1 released.** Extends workstream 4, which resolved MX targets but never asked what they resolved *to*: a host answering only loopback or private space still reported as healthy. 0.9.1 adds address-scope classification and names an address literal and a null-MX conflict for what they are, at no query cost. 0.9.2 adds forward-confirmed reverse lookups to find a vanity MX that has fallen behind its provider's address set. Its privacy review was conducted before any implementation and accepted on 2026-09-05; measured through the shipping code, it costs 8 additional queries across the corpus's 80 background domains — 0 for a domain whose MX hosts are provider-named — and 4 for the dedicated case that exercises it. Spec `1.6`; implemented with review corrections applied, in code review, not released. | [0.9.1 and 0.9.2](docs/specs/mx-host-validity.md) |
...
Spec: [`docs/specs/mx-host-validity.md`](docs/specs/mx-host-validity.md) — `1.6`.
Released as `v0.9.1`. 0.9.2 is specified in the same document, is implemented on this branch, and is in code review — not released.
